...
void Root::build() {
    ...
    while (true) {
        ...
        if(n == 0 && m == 0) break;
            ...
            Base *obj;
            obj = find("obj");
            obj->set_connect(Root::getSignal(1), this, Root::getHandler(1));
            Note *note = new Note;
...
void Root::p_signal(string &text) {
    if ((n < 1 || n > 10 || m < 1 || m > 10) ||
        (!((int) digit > 96 && (int) digit < 123) && !((int) digit > 64 && (int) digit < 91))) {
        text = "error";
    } else {
        ifstream getstring("field.txt");
        int count = 0;
    ...
    void Root::p_handler(string text)
    {
        if(text != "0")
        {
            ...
            firstTxt.open("field.txt", ifstream::in);
            ...
            secondTxt.close();
            fstream ofs, ifs;
            ofs.open("field.txt", ofstream::out | ofstream::trunc);
